...
/**
 * Like customerFetch, but returns null instead of throwing.
 *
 * For data the page can live without — anything behind an optional access
 * scope, above all. Never use it for the query a page actually needs.
 */
export async function customerFetchOptional(query, variables = {}) {
	try {
		return await customerFetch(query, variables)
	} catch (error) {
		console.warn('Optional Customer Account query failed:', error.message)
		return null
	}
}
