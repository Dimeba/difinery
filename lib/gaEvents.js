...
/**
 * Store GCLID in session storage for cross-page tracking
 * Call this on initial page load
 */
export const preserveGclid = () => {
	...
	if (gclid) {
		...
		// Also store in cookie for better persistence (90 days)
		document.cookie = `gclid=${gclid}; path=/; max-age=7776000; SameSite=Lax`
	}
}

/**
 * Get stored GCLID from session storage or cookie
 * @returns {string|null} GCLID value or null
 */
